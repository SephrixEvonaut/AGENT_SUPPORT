...
"@
      ...
      `;
            ...
            logger.error(`Failed to set Discord volume: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
     ...
     */
    async setVolumeWithNircmd(percentage) {
     ...
     */
    async setSystemVolume(percentage) {
    ...
    `;
        ...
        const simpleCommand = `powershell -Command "[audio]::Volume = ${percentage / 100}"`;
        try {
            await execAsync(simpleCommand);
        }
        catch {
            // Ultimate fallback: Use sndvol
            logger.warn("PowerShell audio control failed, volume change may not work");
        }
    }
    /**
    ...
    async setMicMute(mute) {
        ...
        "`;
            ...
            logger.error(`Failed to ${mute ? "mute" : "unmute"} microphone: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
    }
    /**
     ...
     */
    async pressDiscordMicToggle(hotkey = "CTRL+SHIFT+M") {
     ...
     */
    async pressDiscordDeafenToggle(hotkey = "CTRL+SHIFT+D") {
...
// Singleton instance
let discordControllerInstance = null;
export function getDiscordController() {
