...
const logger = {
    info: (msg) => console.log(`[GCD] ${msg}`),
    debug: (msg) => {
        if (process.env.DEBUG_GCD)
            console.log(`[GCD:DEBUG] ${msg}`);
    },
    warn: (msg) => console.warn(`[GCD:WARN] ${msg}`),
    error: (msg) => console.error(`[GCD:ERROR] ${msg}`),
};
// ============================================================================
// CONFIGURATION
// ============================================================================
 ...
 */
export const ABILITY_COOLDOWNS_MS = {
    CRUSHING_BLOW: 7000,
    FORCE_SCREAM: 11000,
    AEGIS_ASSAULT: 11000,
    VICIOUS_THROW: 9000,
...
export class GCDManager {
     ...
     */
    perAbilityCooldownsEnabled = true;
    constructor() {
        logger.info(`Initialized - GCD: ${GCD_DURATION_MS}ms, Abilities: ${GCD_ABILITIES.size}`);
    }
    /**
     * Enable or disable per-ability cooldown tracking.
     * When disabled, only GCD (1.275s) is enforced between abilities.
     */
    setPerAbilityCooldownsEnabled(enabled) {
     ...
     */
    resetShortCooldowns(maxDurationMs = 20000) {
        ...
        if (resetAbilities.length > 0) {
            logger.info(`Reset ${resetAbilities.length} short cooldowns: ${resetAbilities.join(", ")}`);
        }
        else {
     ...
     */
    detectGCDAbility(binding) {
        // First check if binding has explicit gcdAbility field
        if (binding.gcdAbility) {
            return binding.gcdAbility.toUpperCase();
        }
    ...
    isAbilityOnCooldown(abilityName) {
        ...
        if (!cooldownMs)
            return false; // No specific cooldown for this ability
        const elapsed = Date.now() - lastUsed;
     ...
     */
    getAbilityCooldownRemaining(abilityName) {
        const upperName = abilityName.toUpperCase();
        const lastUsed = this.abilityCooldowns.get(upperName);
        if (!lastUsed)
            return 0;
    ...
    tryExecute(binding) {
        ...
        if (this.isAbilityOnCooldown(gcdAbility)) {
            ...
            logger.debug(`${gcdAbility} on cooldown (${(remaining / 1000).toFixed(1)}s remaining)`);
        ...
        if (this.isGCDActive()) {
            ...
            logger.debug(`GCD active (${remaining}ms remaining) - queueing ${binding.name}`);
            this.queueSequence(binding, gcdAbility);
    ...
    // INTERNAL METHODS
    // ==========================================================================
    queueSequence(binding, gcdAbility) {
        this.gcdQueue.push({
            binding,
            queuedAt: Date.now(),
            gcdAbility,
        });
        logger.debug(`Queued: ${binding.name} (${gcdAbility}) - queue size: ${this.gcdQueue.length}`);
    }
    executeGCDBinding(binding, gcdAbility) {
        // Start GCD timer
    ...
    executeNonGCDBinding(binding) {
        if (this.executeCallback) {
            this.executeCallback(binding);
        }
    }
    startGCD(abilityName) {
        const now = Date.now();
        const upperName = abilityName.toUpperCase();
        // Set GCD state
        this.gcdState = {
    ...
    processQueue() {
        ...
        logger.debug(`Processing queue: most recent = ${this.gcdQueue[0].binding.name}`);
        ...
        for (const entry of this.gcdQueue) {
            if (!this.isAbilityOnCooldown(entry.gcdAbility)) {
                logger.info(`Executing queued: ${entry.binding.name} (${entry.gcdAbility})`);
                ...
                logger.debug(`Skipping ${entry.gcdAbility} - on cooldown (${(remaining / 1000).toFixed(1)}s)`);
            }
        }
 ...
 */
export function getGestureFallback(triggeredGesture, hasBinding) {
    // Extract base (single, double, triple, quadruple) and suffix
    const longMatch = triggeredGesture.match(/^(single|double|triple|quadruple)_long$/);
    const superLongMatch = triggeredGesture.match(/^(single|double|triple|quadruple)_super_long$/);
    if (longMatch) {
        // "long" triggered - try "super_long"
        const base = longMatch[1];
        const fallback = `${base}_super_long`;
...
// SINGLETON INSTANCE
// ============================================================================
let gcdManagerInstance = null;
export function getGCDManager() {
    if (!gcdManagerInstance) {
        gcdManagerInstance = new GCDManager();
    }
    return gcdManagerInstance;
}
export function resetGCDManager() {
    if (gcdManagerInstance) {
...
//# sourceMappingURL=gcdManager.js.map