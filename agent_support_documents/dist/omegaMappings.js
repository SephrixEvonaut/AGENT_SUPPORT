 ...
 */
export function buildOmegaBindingLookup(bindings = OMEGA_BINDINGS) {
 ...
 */
export function getOmegaBinding(lookup, inputKey, gesture) {
 ...
 */
export function omegaBindingToMacro(binding) {
    const sequence = binding.sequence.map((step) => {
        const result = {
            key: step.key || "",
 ...
 */
export function exportOmegaProfile() {
    return {
        name: "SWTOR Vengeance Juggernaut - Omega Profile",
        description: "Omega gesture system with 4-gesture detection (quick/long/quick_toggle/long_toggle)",
        system: "omega",
        gestureSettings: {
