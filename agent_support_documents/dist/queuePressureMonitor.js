...
export class QueuePressureMonitor {
     ...
     */
    recordOutput(abilityName, keyPressed, blockingDurationMs, isEchoHit = false, isRStream = false) {
        const now = Date.now();
        ...
        const recentEvents = this.events.filter((e) => now - e.timestamp < 200).length;
        if (recentEvents > 3) {
    ...
    startSpike(timestamp) {
        ...
        console.log(`⚠️ PRESSURE SPIKE #${this.spikeCount} detected! Pressure: ${this.currentPressure.toFixed(1)}`);
    ...
    endSpike() {
        ...
        console.log(`✅ Spike #${this.spikeCount} resolved after ${spikeDuration}ms`);
     ...
     */
    updateAbilityStats(abilityName, pressureContribution) {
        ...
        stats.maxSinglePressure = Math.max(stats.maxSinglePressure, pressureContribution);
    }
    /**
     * Track ability sequences for pattern detection
     */
    trackSequence(abilityName, timestamp) {
        // Add to recent abilities
        this.recentAbilities.push({ name: abilityName, timestamp });
        // Prune old entries
        this.recentAbilities = this.recentAbilities.filter((a) => timestamp - a.timestamp < SEQUENCE_WINDOW_MS);
        ...
        if (this.recentAbilities.length >= 2) {
            ...
            const sequenceEvents = this.events.filter((e) => timestamp - e.timestamp < 500 && lastThree.includes(e.abilityName));
            const sequencePressure = sequenceEvents.reduce((sum, e) => sum + e.pressureContribution, 0);
     ...
     */
    analyzeSequenceForSpike() {
        const spikeTriggerAbilities = this.recentAbilities
            .slice(-5)
            .map((a) => a.name);
        for (const [key, pattern] of this.sequencePatterns) {
            const patternInSpike = pattern.abilities.every((a) => spikeTriggerAbilities.includes(a));
            if (patternInSpike) {
    ...
    takeSnapshot() {
        const now = Date.now();
        const recentEvents = this.events.filter((e) => now - e.timestamp < PRESSURE_WINDOW_MS);
        // Calculate top contributors in this window
        const contributionByAbility = new Map();
        for (const event of recentEvents) {
            const current = contributionByAbility.get(event.abilityName) || 0;
            contributionByAbility.set(event.abilityName, current + event.pressureContribution);
        }
        ...
        const estimatedRecoveryMs = this.currentPressure > 0
     ...
     */
    shouldThrottleAbility(abilityName) {
        ...
        if (stats && stats.appearsInSpikes > 2) {
            const spikeRatio = stats.appearsInSpikes / Math.max(1, stats.totalOutputs);
            if (spikeRatio > 0.1) {
    ...
    generateReport() {
        ...
        const abilityStats = Array.from(this.abilityPressure.values()).sort((a, b) => b.appearsInSpikes - a.appearsInSpikes);
        ...
        const avgPressure = this.snapshots.length > 0
        ...
        return {
            ...
            totalPressure: this.events.reduce((sum, e) => sum + e.pressureContribution, 0),
            peakPressure: this.peakPressure,
    ...
    printSummary() {
        ...
        console.log(`📈 Peak Pressure: ${report.peakPressure.toFixed(1)} (at ${new Date(report.peakTimestamp).toLocaleTimeString()})`);
        ...
        for (const stats of report.abilityStats.slice(0, 8)) {
            const spikeIndicator = stats.appearsInSpikes > 0 ? ` ⚠️${stats.appearsInSpikes} spikes` : "";
            console.log(`   ${stats.abilityName}: ${stats.totalOutputs} outputs, avg ${stats.avgPressurePerOutput.toFixed(1)} pressure${spikeIndicator}`);
        }
        if (report.problematicSequences.length > 0) {
            ...
            for (const seq of report.problematicSequences.slice(0, 5)) {
                ...
                console.log(`      ${seq.occurrences}x, avg pressure: ${seq.avgPressureGenerated.toFixed(1)}, spike correlation: ${(seq.spikeCorrelation * 100).toFixed(0)}%`);
            }
        }
    ...
    getStatusString() {
        ...
        const indicator = level < 0.3 ? "🟢" : level < 0.6 ? "🟡" : level < 0.8 ? "🟠" : "🔴";
        return `${indicator} Pressure: ${(level * 100).toFixed(0)}% | Spikes: ${this.spikeCount} | Adaptive Delay: +${this.getAdaptiveDelay()}ms`;
    }
}
// Singleton instance
let monitorInstance = null;
export function getQueuePressureMonitor() {
    if (!monitorInstance) {
        monitorInstance = new QueuePressureMonitor();
    }
    return monitorInstance;
}
export function resetQueuePressureMonitor() {
    monitorInstance = new QueuePressureMonitor();
}
//# sourceMappingURL=queuePressureMonitor.js.map